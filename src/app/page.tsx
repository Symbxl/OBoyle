...
import { portfolio, team, gardenCenter, company } from "@/lib/media";
...
export default function HomePage() {
  return (
    <>
      {/* HERO */}
      <section className="relative isolate overflow-hidden lg:min-h-[88vh] lg:flex lg:items-center">
        <HeroSlideshow />
        <div className="container-x py-20 md:py-28 lg:py-24 grid gap-10 lg:grid-cols-[1.1fr_1fr] lg:items-center">
          <div className="text-white">
            ...
            <div className="mt-8 grid grid-cols-3 gap-6 max-w-md">
              <Stat value={company.yearsExperience} label="Years" />
              <Stat value="2nd" label="Generation" />
              <Stat value="LIC" label={`#${company.license.slice(0, 7)}…`} />
            </div>
          </div>
...
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div>
      <div className="font-serif text-3xl font-bold text-sand-300">{value}</div>
      <div className="text-xs uppercase tracking-widest text-moss-100/80">{label}</div>
    </div>
  );
}
